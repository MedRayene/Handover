...
CONF_DIR="$REPO_DIR/configs/oai/cudu"
UE_CONF="$REPO_DIR/configs/oai/ue.conf"
...
echo "=== 2. Vérification que le core (vplmn) et grafana tournent ==="
docker ps --format '{{.Names}}' | grep -E "vplmn-core|grafana" || {
  echo "Core/Grafana non détectés, lance d'abord :"
  echo "  ./scripts/02-start-cores.sh"
  exit 1
}
